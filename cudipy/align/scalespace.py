"""GPU-based ScaleSpace classes.

This ScaleSpace class is exactly as in DiPy except that the images are GPU
arrays. All affines, shapes, spacings remain on the CPU for efficiency.
...
class ScaleSpace(object):
    def __init__(
        ...
        for i in range(1, num_levels):
            ...
            filtered = ndimage.gaussian_filter(image, sigmas)
...
class IsotropicScaleSpace(ScaleSpace):
    def __init__(
        ...
        for i in range(1, self.num_levels):
            ...
            # Filter along each direction with the appropriate sigma
            filtered = ndimage.gaussian_filter(image.astype(np.float64), new_sigmas)
